...
fn hundred_thousand_rows_create_populate_query_update_delete_reopen() {
...

    {
        ...
        db.execute("CREATE TABLE t (id INTEGER PRIMARY KEY, val INTEGER NOT NULL)")
            .unwrap();
        for i in 0..100_000i64 {
            db.execute(&format!(
                "INSERT INTO t (id, val) VALUES ({i}, {})",
                i % 1000
            ))
            .unwrap();
        }
        ...
        assert_eq!(
            ...
            ExecResult::Rows {
                columns: vec!["id".into()],
                rows: vec![vec![Value::Integer(54321)]]
            }
        );

        assert_eq!(
            db.execute("UPDATE t SET val = 9999 WHERE id = 1").unwrap(),
        ...
        assert!(
            dump.lines().any(|l| l.trim_start().starts_with("internal")),
            "100k rows must produce a multi-level tree"
        );
    }
    ...
    assert_eq!(
        ...
        ExecResult::Rows {
            ...
            rows: vec![vec![Value::Integer(9999)]]
        }
    );
