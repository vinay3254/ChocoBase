...
async fn realtime_changefeed_streams_insert_update_delete_events() {
    ...
    .await
    .unwrap();
    let resp = read_response(&mut sub_reader, &mut sub_buf)
        .await
        .unwrap()
        .unwrap();
    ...
    write_request(
        &mut mut_writer,
        &Request::Query {
            sql: "CREATE TABLE customers (id INTEGER PRIMARY KEY, name TEXT)".into(),
        },
    )
    .await
    .unwrap();
    ...
    write_request(
        &mut mut_writer,
        &Request::Query {
            sql: "INSERT INTO customers (id, name) VALUES (1, 'Alice')".into(),
        },
    )
    .await
    .unwrap();
    ...
    let insert_event = timeout(
        Duration::from_secs(2),
        read_response(&mut sub_reader, &mut sub_buf),
    )
    .await
    .unwrap()
    .unwrap()
    ...
    write_request(
        &mut mut_writer,
        &Request::Query {
            sql: "UPDATE customers SET name = 'Alicia' WHERE id = 1".into(),
        },
    )
    .await
    .unwrap();
    ...
    match update_event {
        Response::Event(evt) => {
            ...
            assert_eq!(
                evt.new_row,
                Some(vec![Value::Integer(1), Value::Text("Alicia".into())])
            );
        }
        other => panic!("expected ChangeEvent, got {other:?}"),
    }
    ...
    write_request(
        &mut mut_writer,
        &Request::Query {
            sql: "DELETE FROM customers WHERE id = 1".into(),
        },
    )
    .await
    .unwrap();
...
async fn realtime_changefeed_respects_table_subscription_filters() {
    ...
    .unwrap();
    assert_eq!(
        read_response(&mut orders_reader, &mut orders_buf)
            .await
            .unwrap()
            .unwrap(),
        Response::Subscribed
    ...
    write_request(&mut all_writer, &Request::Subscribe { table: None })
        .await
        .unwrap();
    assert_eq!(
        read_response(&mut all_reader, &mut all_buf)
            .await
            .unwrap()
            .unwrap(),
        Response::Subscribed
    ...
    write_request(
        &mut mut_writer,
        &Request::Query {
            sql: "CREATE TABLE items (id INTEGER PRIMARY KEY, title TEXT)".into(),
        },
    )
    .await
    .unwrap();
    ...
    write_request(
        &mut mut_writer,
        &Request::Query {
            sql: "INSERT INTO items (id, title) VALUES (10, 'Book')".into(),
        },
    )
    .await
    .unwrap();
    ...
    assert!(
        orders_event.is_err(),
        "Orders subscriber should not receive items event"
    );
}
...
async fn realtime_changefeed_buffers_events_until_commit_and_discards_on_rollback() {
    ...
    write_request(
        &mut mut_writer,
        &Request::Query {
            sql: "INSERT INTO accounts (id, balance) VALUES (1, 500)".into(),
        },
    )
    .await
    .unwrap();
    ...
    let rolled_back_event = timeout(
        Duration::from_millis(200),
        read_response(&mut sub_reader, &mut sub_buf),
    )
    .await;
    assert!(
        rolled_back_event.is_err(),
        "Rolled back transaction must NOT emit change events"
    );
    ...
    write_request(
        &mut mut_writer,
        &Request::Query {
            sql: "INSERT INTO accounts (id, balance) VALUES (2, 1000)".into(),
        },
    )
    .await
    .unwrap();
    ...
    assert!(
        ...
        "Events must not emit prior to COMMIT"
    );
    ...
    match commit_event {
        Response::Event(evt) => {
            ...
            assert_eq!(
                evt.new_row,
                Some(vec![Value::Integer(2), Value::Integer(1000)])
            );
        }
